...
static int
BottleneckLinkBufferSize (int times)
{
  int bottleneck=100;
  double rtt = (5+10+5)/1000.0*2;
  double bdp = ceil(bottleneck*rtt*1024*1024/8);
  int maxlen = ceil(bdp/1448)*times;
  // std::cout<<"bottleneck: "<<bottleneck<<" rtt: "<<rtt<<" bdp: "<<bdp<<" maxlen: "<<maxlen<<std::endl;
  return maxlen;
}
...
TraceThroughput (Ptr<FlowMonitor> monitor)
{
  ...
  if(itr!=stats.end()){
    thr <<  curTime.GetSeconds () << " " << 8 * (itr->second.rxBytes - prev) / (1000 * 1000 * (curTime.GetSeconds () - prevTime.GetSeconds ()));
    prev = itr->second.rxBytes;
  }
  itr = stats.find(3);
  if(itr!=stats.end()){
    thr << " " << 8 * (itr->second.rxBytes - prev2) / (1000 * 1000 * (curTime.GetSeconds () - prevTime.GetSeconds ())) << std::endl;
    prev2 = itr->second.rxBytes;
  }
  else{
    thr << " 0"<< std::endl;
  }
...
static void CwndTracer (Ptr<OutputStreamWrapper> stream, uint32_t oldval, uint32_t newval)
{
  *stream->GetStream () << Simulator::Now ().GetSeconds () << " " << newval / 1448.0 << std::endl;
  // std::cout<<newval / 1448.0 << std::endl;
}
...
int main (int argc, char *argv [])
{
  ...
  std::string bottleneckLinkSpeed = "100Mbps";
  int bottleneckbuffersize = 1;

  CommandLine cmd (__FILE__);
  cmd.AddValue ("tcpTypeId", "Transport protocol to use: TcpNewReno, TcpBbr, TcpCubic", tcpTypeId);
  cmd.AddValue ("delAckCount", "Delayed ACK count", delAckCount);
  cmd.AddValue ("enablePcap", "Enable/Disable pcap file generation", enablePcap);
  cmd.AddValue ("stopTime", "Stop time for applications / simulation time will be stopTime + 1", stopTime);
  cmd.AddValue ("blspeed", "Bottleneck speed", bottleneckLinkSpeed);
  cmd.AddValue ("buffersize", "Bottleneck buffer size(times of BDP)", bottleneckbuffersize);
  cmd.Parse (argc, argv);
  ...
  // Config::SetDefault ("ns3::TcpSocketBase::MaxWindowSize", UintegerValue (1000));
  
  Config::SetDefault ("ns3::DropTailQueue<Packet>::MaxSize", QueueSizeValue (QueueSize ("1p")));

  int lyj_ql = BottleneckLinkBufferSize(bottleneckbuffersize);
  std::string config_ql = std::to_string(lyj_ql)+"p";
  // std::cout<<config_ql<<std::endl;
  // return 0;
  // Config::SetDefault (queueDisc + "::MaxSize", QueueSizeValue (QueueSize ("100p")));
  Config::SetDefault (queueDisc + "::MaxSize", QueueSizeValue (QueueSize (config_ql)));
  ...
  bottleneckLink.SetDeviceAttribute  ("DataRate", StringValue (bottleneckLinkSpeed));
  ...
  NetDeviceContainer senderEdge2 = edgeLink.Install (sender2.Get (0), routers.Get (0));
  NetDeviceContainer receiverEdge2 = edgeLink.Install (routers.Get (1), receiver2.Get (0));
  ...
  sourceApps2.Start (Seconds (5));
  // Hook trace source after application starts
  // Simulator::Schedule (Seconds (0.1) + MilliSeconds (1), &TraceCwnd, 0, 0);
  sourceApps2.Stop (Seconds (40));
  ...
  system  (("cd "+dir+"; gnuplot -c gnuplotScriptCwnd").c_str ());
  system  (("cd "+dir+"; gnuplot -c gnuplotScriptThroughput").c_str ());
  system  (("cd "+dir+"; gnuplot -c gnuplotScriptQueueSize").c_str ());
  return 0;
}
